...
const FrequencyBarsVisualizer: React.FC = () => {
  ...
  const [realTimeFrequencies, setRealTimeFrequencies] = useState<number[]>([]);
    ...
    const startAnalyzing = async () => {
      try {
        ...
        const visualize = () => {
          ...
          // Calcular frecuencias reales para cada barra
          const frequencies: number[] = [];
          const step = Math.floor(dataArrayRef.current.length / barCount);
          for (let i = 0; i < barCount; i++) {
            const binIndex = i * step;
            const frequency = Math.round((binIndex * sampleRate) / analyserRef.current.fftSize);
            frequencies.push(frequency);
          }
          setRealTimeFrequencies(frequencies);
      ...
      <div className="text-center mb-8">
        ...
        {showHelp && (
          <div className="mt-4 p-4 bg-gray-700 rounded-lg text-left text-sm">
            ...
            <p className="mt-2 text-blue-400">Las barras muestran frecuencias reales desde 0 Hz hasta ~22 kHz</p>
          </div>
        )}
      </div>
        ...
        >
          {Array.from({ length: frequencyBarsCount }).map((_, i) => (
            <div
              ...
              title={realTimeFrequencies[i] ? `${realTimeFrequencies[i]} Hz` : 'Calculando...'}
            ></div>
          ))}
        </div>

        {/* Mostrar frecuencias reales */}
        <div className="bg-gray-700 p-4 rounded-lg mb-4">
          <h3 className="text-sm font-semibold text-gray-400 mb-2">RANGO DE FRECUENCIAS</h3>
          ...
          <div className="text-center text-xs text-blue-400 mt-2">
            Cada barra representa aproximadamente {Math.round(22000 / frequencyBarsCount)} Hz
          </div>
        </div>
